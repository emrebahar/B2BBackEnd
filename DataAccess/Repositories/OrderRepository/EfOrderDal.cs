...
namespace DataAccess.Repositories.OrderRepository
{
    public class EfOrderDal : EfEntityRepositoryBase<Order, SimpleContextDb>, IOrderDal
    {
            ...
            using (var context = new SimpleContextDb())
            {
                var result = from order in context.Orders
                             ...
                             select new OrderDto
                             {
                                 ...
                                 Status = order.Status,
                                 Quantity = context.OrderDetails.Where(p => p.OrderId == order.Id).Sum(x => x.Quantity),
                                 Total = context.OrderDetails.Where(p => p.OrderId == order.Id).Sum(x => (x.Price) * (x.Quantity))
                             };
        ...
        public async Task<List<OrderDto>> GetListByCustomerIdDto(int customerId)
        {
            using (var context = new SimpleContextDb())
            {
                var result = from order in context.Orders.Where(x=>x.CustomerId == customerId)
                             join customer in context.Customers on order.CustomerId equals customer.Id
                             select new OrderDto
                             {
                                 Id = order.Id,
