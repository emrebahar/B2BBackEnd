...
namespace Business.Authentication
{
    public class AuthManager : IAuthService
    {
        ...
        public AuthManager(IUserService userService, ITokenHandler tokenHandler, ICustomerService cusomerService)
        {
            _userService = userService;
            _tokenHandler = tokenHandler;
            _cusomerService = cusomerService;
        }
        ...
        public async Task<IDataResult<AdminToken>> UserLogin(LoginAuthDto loginDto)
        {
            ...
            if (user == null)
            {
                return new ErrorDataResult<AdminToken>("Kullanıcı maili bulunamadı");
            }
        ...
        {
            var customer = await _cusomerService.GetByEmail(customerLoginDto.Email);
            if (customer == null)
            {
                return new ErrorDataResult<CustomerToken>("Kullanıcı maili bulunamadı");
            }
